...
Gmail「使用 Google 登录」发布版可配置仓库 Secrets：

- `GOOGLE_OAUTH_CLIENT_ID`：Google Cloud 中创建的 **Desktop app** OAuth 客户端 ID，用于 Gmail OAuth2 / XOAUTH2。没有配置时，本地开发仍可在添加 Gmail 账户时手填 Client ID。
- `GOOGLE_OAUTH_CLIENT_SECRET`：同一个 **Desktop app** OAuth 客户端的 Client Secret。部分 Google OAuth 客户端在授权码换 token 时会要求该值。
