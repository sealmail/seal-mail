...
### v25（新邮件自动应用过滤规则 + 单连接批量整理 / v0.1.41）

- [x] 自动筛选：`sync_messages` 同步 INBOX 时对**新到邮件**自动执行规则整理
      （IMAP IDLE 推送 → 前端刷新 → sync 即触发；POP3 同理，本地虚拟目录纯本地操作）。
      之前规则只有「立即整理」按钮一个入口。
- [x] 整理提速：原来每封命中邮件各开两条全新 TLS IMAP 连接（移动一条 + 标已读一条）。
      新增 `filters::plan_moves`（纯函数：按规则分组出移动计划，同目标+同标已读合并，
      规则按顺序第一条命中生效，跳过原地移动）+ `imap_client::organize_messages`
      （单条连接完成全部分组，uidset 批量命令）。N 封从 2N 条连接降到 1 条。
