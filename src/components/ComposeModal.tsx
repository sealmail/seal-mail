...
export function ComposeModal(p: Props) {
  ...
  async function pickAttachments() {
    const picked = await openFileDialog({ multiple: true, title: t("选择附件") });
    if (!picked) return;
          ...
          {step === 1 && (
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", padding: "18px 0" }}>
              ...
              <div style={{ fontSize: 15, fontWeight: 700, color: "var(--ink)", marginTop: 22 }}>
                {sign && isLedger ? t("在你的 Ledger 上确认签名") : sign ? t("正在签名并发送") : t("正在发送")}
              </div>
              <div style={{ fontSize: 12.5, color: "var(--mut)", marginTop: 6, maxWidth: 360, lineHeight: 1.6 }}>
                {sign && isLedger
                  ? t("核对设备屏幕上的内容摘要，按下两侧按钮确认。私钥永不离开硬件。")
                  : sign
                    ? t("正文哈希已计算，正用你的本地密钥盖印，随后通过 SMTP 投递。私钥不会离开本机。")
                    : t("正在通过 SMTP 投递。")}
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 18, color: "var(--mut-2)", fontSize: 12 }}>
                <span className="pulse-dot" /> {sign && isLedger ? t("等待硬件确认…") : t("正在投递…")}
              </div>
            </div>
          )}
          ...
          {step === 2 && result && (
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", padding: "14px 0" }}>
              ...
              <div style={{ fontSize: 12.5, color: "var(--mut)", marginTop: 7, maxWidth: 380, lineHeight: 1.6 }}>
                {result.signed
                  ? t("收件人若使用 SealMail，将看到这枚完整封印——证明邮件确实出自你的密钥，且内容未被改动。普通邮箱则正常收信。")
                  : t("邮件已通过 SMTP 正常发出。")}
              </div>
              <div
                style={{
